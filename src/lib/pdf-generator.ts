...
// Define the structure for the content to be added to the PDF
type PdfContent = {
  title: string;
  content: string | { section: string; text: string }[];
};
...
const drawFooter = (doc: jsPDF) => {
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.setTextColor(COLORS.secondary);
        doc.text('This document was auto-generated by StudioFlow AI.', PAGE_MARGIN, pageHeight - 10);
...
export const generatePdf = (
  ...
  content.forEach((item, index) => {
    ...
    const sectionTitleHeight = 10; // Rough estimate for title height with spacing
    if (currentY + sectionTitleHeight > contentEndY) {
      doc.addPage();
      currentY = contentStartY + CONTENT_PADDING;
      drawHeader(doc, businessData, documentTitle, generatedBy);
      doc.roundedRect(PAGE_MARGIN, contentStartY, pageWidth - PAGE_MARGIN * 2, contentEndY - contentStartY, 3, 3, 'S');
    }
    ...
    splitText.forEach((line: string) => {
        if (currentY + 4 > contentEndY) { // 4 is approx line height
            doc.addPage();
            currentY = contentStartY + CONTENT_PADDING;
            drawHeader(doc, businessData, documentTitle, generatedBy);
            doc.roundedRect(PAGE_MARGIN, contentStartY, pageWidth - PAGE_MARGIN * 2, contentEndY - contentStartY, 3, 3, 'S');
